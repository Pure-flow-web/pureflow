  ...
  return (
    <li className={`flex items-start gap-3 p-3.5 transition-all bg-white/50 dark:bg-gray-800/40 border-l-4 rounded-lg shadow-soft hover:shadow-md ${priorityStyles[task.priority]} ${task.completed ? "opacity-60" : ""}`}>
      <button onClick={handleToggle} className="flex-shrink-0 mt-0.5 group" aria-label={`Mark task ${task.completed ? 'incomplete' : 'complete'}`}>
        <div className={`w-5 h-5 flex items-center justify-center rounded-md border-2 transition-all duration-200 ${task.completed ? "bg-purple-600 border-purple-600" : "border-gray-300 dark:border-gray-600 group-hover:border-purple-500"}`}>
          {task.completed && <Check className="w-4 h-4 text-white" />}
        </div>
      </button>
      <div className="flex-1">
        <p className={`font-medium text-gray-800 dark:text-gray-200 ${task.completed ? "line-through text-gray-500 dark:text-gray-400" : ""}`}>{task.title}</p>
      ...
      <div className="flex items-center gap-0.5">
        <button onClick={() => setTaskToEdit(task)} className="p-2 text-gray-400 rounded-full h-8 w-8 hover:bg-gray-500/10 hover:text-purple-500" aria-label="Edit task"><Edit className="w-4 h-4" /></button>
    ...
    <>
      <div className="p-5 bg-white/30 dark:bg-gray-800/20 backdrop-blur-xl border border-black/5 dark:border-white/5 rounded-xl shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
          <h2 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Tasks</h2>
          <div className="flex items-center gap-2">
            <button onClick={handleOpenModal} className="inline-flex items-center justify-center h-9 gap-2 px-3 text-sm font-semibold text-white bg-purple-600 rounded-lg hover:bg-purple-700"><Plus className="w-4 h-4" /> Add Task</button>
        ...
        <div className="flex items-center gap-2 mb-4">
          <button onClick={() => setFilter('pending')} className={`px-3 py-1 text-sm font-medium rounded-full transition-colors ${filter === 'pending' ? 'bg-purple-600 text-white' : 'bg-gray-200/50 text-gray-600 dark:text-gray-300 dark:bg-gray-700/50 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>Pending</button>
          <button onClick={() => setFilter('completed')} className={`px-3 py-1 text-sm font-medium rounded-full transition-colors ${filter === 'completed' ? 'bg-purple-600 text-white' : 'bg-gray-200/50 text-gray-600 dark:text-gray-300 dark:bg-gray-700/50 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>Completed</button>
          <button onClick={() => setFilter('all')} className={`px-3 py-1 text-sm font-medium rounded-full transition-colors ${filter === 'all' ? 'bg-purple-600 text-white' : 'bg-gray-200/50 text-gray-600 dark:text-gray-300 dark:bg-gray-700/50 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>All</button>
        </div>
        ...
        {filteredTasks.length === 0 ? (
          <div className="py-12 text-center border-2 border-dashed rounded-lg border-gray-300/50 dark:border-gray-600/50">
            ...
            <h3 className="mt-3 text-lg font-medium text-gray-800 dark:text-gray-200">All Clear!</h3>
