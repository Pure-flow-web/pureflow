        ...
        {notes.length === 0 && !editingNoteId ? (
          ...
          notes.sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(note => (
            <div key={note.id} className={`flex items-start gap-3 p-3 transition-all bg-white/50 dark:bg-gray-800/40 rounded-lg shadow-soft ${note.completed ? "opacity-60" : ""}`}>
              <button onClick={() => toggleNote(note.id)} className="flex-shrink-0 mt-0.5 group"><div className={`w-5 h-5 flex items-center justify-center rounded-md border-2 transition-all duration-200 ${note.completed ? "bg-accent-blue border-accent-blue" : "border-gray-300 dark:border-gray-600 group-hover:border-accent-blue"}`}>{note.completed && <Check className="w-4 h-4 text-white" />}</div></button>
              ...
              <div className="flex items-center gap-0.5">
                <button onClick={() => handleEdit(note)} className="p-2 text-gray-400 rounded-full h-8 w-8 hover:bg-gray-500/10 hover:text-accent-blue"><Edit className="w-4 h-4" /></button>
